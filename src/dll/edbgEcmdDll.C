...
uint32_t queryConfigExist(const ecmdChipTarget &i_target,
                          ecmdQueryData &o_queryData,
                          ecmdQueryDetail_t i_detail, bool i_allowDisabled);
uint32_t queryConfigExistCages(const ecmdChipTarget &i_target,
                               std::list<ecmdCageData> &o_cageData,
                               ecmdQueryDetail_t i_detail,
                               bool i_allowDisabled);
uint32_t queryConfigExistNodes(const ecmdChipTarget &i_target,
                               std::list<ecmdNodeData> &o_nodeData,
                               ecmdQueryDetail_t i_detail,
                               bool i_allowDisabled);
uint32_t queryConfigExistSlots(const ecmdChipTarget &i_target,
                               std::list<ecmdSlotData> &o_slotData,
                               ecmdQueryDetail_t i_detail,
                               bool i_allowDisabled);
uint32_t queryConfigExistChips(const ecmdChipTarget &i_target,
                               std::list<ecmdChipData> &o_chipData,
                               ecmdQueryDetail_t i_detail,
                               bool i_allowDisabled);
uint32_t queryConfigExistChipUnits(const ecmdChipTarget &i_target,
                                   struct pdbg_target *i_pTarget,
                                   std::string class_type,
                                   std::list<ecmdChipUnitData> &o_chipUnitData,
                                   ecmdQueryDetail_t i_detail,
                                   bool i_allowDisabled);
uint32_t queryConfigExistThreads(const ecmdChipTarget &i_target,
                                 struct pdbg_target *i_pTarget,
                                 std::list<ecmdThreadData> &o_threadData,
                                 ecmdQueryDetail_t i_detail,
                                 bool i_allowDisabled);
...
/* #################################################################################################
 */
static uint32_t fetchPdbgInterfaceTarget(const ecmdChipTarget &i_target,
...
static uint32_t fetchPibTarget(const ecmdChipTarget &i_target,
                               struct pdbg_target **o_pibTarget) {
...
static uint32_t fetchCfamTarget(const ecmdChipTarget &i_target,
                                struct pdbg_target **o_pibTarget) {
 ...
 * "mba", etc.) */
static uint32_t findChipUnitType(const ecmdChipTarget &i_target,
                                 uint64_t i_address,
 ...
 */
static inline uint8_t getLogLevelFromEnv(const char *env,
                                         const uint8_t i_logLevel) {
  ...
  try {
    ...
    out.error(EDBG_GENERAL_ERROR, FUNCNAME,
              "Conversion Failure env=%s exception=%s", env, e.what());
  }
  return l_logLevel;
}
...
uint32_t printConfig(configEntry_t &configEntry) {
  ...
  if (configEntry.type == CONFIG_MAP) {
    ...
    for (iter = configEntry.map.begin(); iter != configEntry.map.end();
         iter++) {
  ...
  if (configEntry.type == CONFIG_LIST) {
    ...
    for (iter = configEntry.list.begin(); iter != configEntry.list.end();
         iter++) {
      rc = printConfig(*iter);
      if (rc)
        return rc;
...
uint32_t findConfigEntryValue(configEntry_t &configEntry, std::string &i_key,
                              std::list<configEntry_t> &o_configList) {
...
// We need a search function that at a given level will go thru the children and
// find any that match It should return that as a list That function can then be
// used to find multiple planar, chip, chipunit declarations
uint32_t readCnfg() {
  uint32_t rc = ECMD_SUCCESS;
  ...
  // If we don't have a config file, bail here and don't fail trying to open it
  // below We'll assume the user isn't doing vpd stuff and doesn't need the
  // config
  if (cnfgFile.empty()) {
    return rc;
  }
  ...
  do {
    ...
    case YAML_SEQUENCE_START_EVENT:
      // Sequence starts tell us we have a key out there we can use to create
      // the next level That means where we are is going to be a MAP
      cur->type = CONFIG_MAP;
    ...
    case YAML_MAPPING_START_EVENT:
      // It would be nice if the enum from yaml (map) matched how we have to
      // store this (list) But it doesn't Setup that we are a list
    ...
    case YAML_SCALAR_EVENT:
      // For a scalar, figure out if we are on the key or value part and act
      // appropriately
      if (!keyOrValue) {
  ...
  if (configList.size() == 0) {
    return out.error(EDBG_GENERAL_ERROR, FUNCNAME,
                     "The version entry was not found in the config!\n");
  }
  ...
  if (configList.size() == 0) {
    return out.error(EDBG_GENERAL_ERROR, FUNCNAME,
                     "The vpd section was not found in the config!\n");
  }

  // We did have it, now grab the data we need out of it
  configEntry = configList.front();
  // This configEntry will be list of vpd entries in the config.  They look like
  // this:
  // - target: k0:n0:s0
  //   system-vpd: /some/path/to/an/eeprom
  // - target: k0:n0:s1
  //   system-vpd: /some/path/to/a/second/eeprom
  // We have to loop through the list and verify we have the contents we need in
  // each map
  std::list<configEntry_t>::iterator iter; // To walk the list
  std::map<std::string, configEntry_t>::iterator
      findIter; // To find the contents in the map
  for (iter = configEntry.list.begin(); iter != configEntry.list.end();
       iter++) {
    ...
    if (findIter == iter->map.end()) {
      out.error(FUNCNAME,
                "The \'system-vpd:\' entry wasn't found in \'vpd:\'\n");
      continue;
    } else {
      // Found it, save it for the end
...
uint32_t dllInitDll() {
  ...
  // EDBG_HOME
  // Allow the user to specify it, otherwise dynamically determine it via
  // ECMD_EXE
...
void dllLoadDllRecovery(std::string i_commandLine, uint32_t &io_rc) { return; }
...
std::string dllSpecificParseReturnCode(uint32_t i_returnCode) {
  ...
  /* #define ECMD_ERR_UNKNOWN                        0x00000000 ///< This error
   * code wasn't flagged to which plugin it came from */
  /* #define ECMD_ERR_ECMD                           0x01000000 ///< Error came
   * from eCMD                                        */
  /* #define ECMD_ERR_CRONUS                         0x02000000 ///< Error came
   * from Cronus                                      */
  /* #define ECMD_ERR_IP                             0x04000000 ///< Error came
   * from IP GFW                                      */
  /* #define ECMD_ERR_Z                              0x08000000 ///< Error came
   * from Z GFW                                       */
...
/* #################################################################################################
 */
/* System Query Functions - System Query Functions - System Query Functions -
...
uint32_t queryConfigExist(const ecmdChipTarget &i_target,
  ...
  // From here, we will recursively work our way through all levels of hierarchy
  // in the target
  if (i_target.cageState == ECMD_TARGET_FIELD_VALID ||
      i_target.cageState == ECMD_TARGET_FIELD_WILDCARD) {
    rc = queryConfigExistCages(i_target, o_queryData.cageData, i_detail,
                               i_allowDisabled);
    if (rc)
      return rc;
  }
...
uint32_t queryConfigExistCages(const ecmdChipTarget &i_target,
  ...
  if (i_target.nodeState == ECMD_TARGET_FIELD_VALID ||
    ...
    rc = queryConfigExistNodes(i_target, cageData.nodeData, i_detail,
                               i_allowDisabled);
    if (rc)
      return rc;
  }
...
uint32_t queryConfigExistNodes(const ecmdChipTarget &i_target,
  ...
  if (i_target.slotState == ECMD_TARGET_FIELD_VALID ||
    ...
    rc = queryConfigExistSlots(i_target, nodeData.slotData, i_detail,
                               i_allowDisabled);
    if (rc)
      return rc;
  }
...
uint32_t queryConfigExistSlots(const ecmdChipTarget &i_target,
  ...
  if (i_target.chipTypeState == ECMD_TARGET_FIELD_VALID ||
      i_target.chipTypeState == ECMD_TARGET_FIELD_WILDCARD) {
    rc = queryConfigExistChips(i_target, slotData.chipData, i_detail,
                               i_allowDisabled);
    if (rc)
      return rc;
  }
...
uint32_t queryConfigExistChips(const ecmdChipTarget &i_target,
  ...
  // the memory chips by FAPI_POS.
    ...
    // If the chipUnitType states are set, see what chipUnitTypes are in this
    // chipType
    if (i_target.chipUnitTypeState == ECMD_TARGET_FIELD_VALID ||
        i_target.chipUnitTypeState == ECMD_TARGET_FIELD_WILDCARD) {
      // Look for chipunits
      rc = queryConfigExistChipUnits(i_target, chipTarget, chipData.chipType,
                                     chipData.chipUnitData, i_detail,
                                     i_allowDisabled);
      if (rc)
        return rc;
    }
  ...
  if ((i_target.chipType == "explorer") || (i_target.chipType == "odyssey") ||
    ...
    pdbg_for_each_class_target("ocmb", ocmbTarget) {
      ...
      if (i_target.chipUnitTypeState == ECMD_TARGET_FIELD_VALID ||
          i_target.chipUnitTypeState == ECMD_TARGET_FIELD_WILDCARD) {
        // Look for chipunits
        rc = queryConfigExistChipUnits(i_target, ocmbTarget, chipData.chipType,
                                       chipData.chipUnitData, i_detail,
                                       i_allowDisabled);
        if (rc)
          return rc;
...
uint32_t addChipUnits(const ecmdChipTarget &i_target,
                      struct pdbg_target *i_pTarget, std::string class_name,
                      std::list<ecmdChipUnitData> &o_chipUnitData,
                      ecmdQueryDetail_t i_detail, bool i_allowDisabled) {
  uint32_t rc = ECMD_SUCCESS;
  struct pdbg_target *target;
  ...
  if (class_name == "explorer") {
    ...
    pdbg_for_each_target(class_name.c_str(), i_pTarget, target) {
      ...
      if (i_target.threadState == ECMD_TARGET_FIELD_VALID ||
        ...
        // Look for chipunits
        rc = queryConfigExistThreads(i_target, target, chipUnitData.threadData,
                                     i_detail, i_allowDisabled);
        if (rc)
          return rc;
      }
      o_chipUnitData.push_back(chipUnitData);
...
uint32_t addOdysseyChipUnits(const ecmdChipTarget &i_target,
                             struct pdbg_target *i_pTarget,
...
uint32_t queryConfigExistChipUnits(const ecmdChipTarget &i_target,
  ...
  if (pdbg_get_proc() == PDBG_PROC_P10 || pdbg_get_proc() == PDBG_PROC_PST) {
    if (class_type == ECMD_CHIPT_PROCESSOR || 
      class_type == ECMD_CHIPT_PROC_HUB) {
      for (l_index = 0;
        ...
        if (ChipUnitTable[l_index].pdbgClassType != "pib") {
          rc = addChipUnits(i_target, i_pTarget,
                            ChipUnitTable[l_index].pdbgClassType,
                            o_chipUnitData, i_detail, i_allowDisabled);
          if (rc) {
            out.error(EDBG_GENERAL_ERROR, FUNCNAME,
                      "Failed to add chip unit:%s\n",
    ...
    // Add explorer targets if target class type selected is explorer
      ...
      rc = addChipUnits(i_target, i_pTarget, class_type, o_chipUnitData,
                        i_detail, i_allowDisabled);
      if (rc) {
        return out.error(EDBG_GENERAL_ERROR, FUNCNAME,
                         "Failed to add ocmb chip unit\n");
      }
    } else if (class_type == "odyssey") {
      for (l_index = 0; l_index < (sizeof(OdysseyChipUnitTable) /
                                   sizeof(odyssey_chipUnit_t));
           l_index++) {
        if (OdysseyChipUnitTable[l_index].pdbgClassType != "pib") {
          rc = addOdysseyChipUnits(i_target, i_pTarget,
                                   OdysseyChipUnitTable[l_index].pdbgClassType,
                                   o_chipUnitData, i_detail, i_allowDisabled);
          if (rc) {
            out.error(EDBG_GENERAL_ERROR, FUNCNAME,
                      "Failed to add chip unit:%s\n",
...
uint32_t dllSetConfiguration(const ecmdChipTarget &i_target, std::string i_name,
                             ...
                             std::string i_valueAlpha,
                             uint32_t i_valueNumeric) {
  return ECMD_FUNCTION_NOT_SUPPORTED;
}
...
uint32_t
dllQueryConnectedTargets(const ecmdChipTarget &i_target,
...
/* #########################################################################################
 */
/* Info Query Functions - Info Query Functions - Info Query Functions - Info
 * Query Functions */
/* #########################################################################################
 */
uint32_t dllQueryFileLocation(const ecmdChipTarget &i_target,
                              ecmdFileType_t i_fileType,
                              std::list<ecmdFileLocation> &o_fileLocations,
...
/* ######################################################################### */
void dllOutputError(const char *message) { out.error("ECMD", message); }

void dllOutputWarning(const char *message) { out.warning("ECMD", message); }

void dllOutput(const char *message) { out.print(message); }

/* #########################################################################################
 */
/* Ring Cache Functions - Ring Cache Functions - Ring Cache Functions - Ring
 * Cache Functions */
/* #########################################################################################
 */
uint32_t dllEnableRingCache(const ecmdChipTarget &i_target) {
  return out.error(ECMD_FUNCTION_NOT_SUPPORTED, FUNCNAME,
...
bool dllQueryTraceMode(ecmdTraceType_t i_type) { return false; }
...
uint32_t dllGetChipData(const ecmdChipTarget &i_target, ecmdChipData &o_data) {
  ...
  if (pdbg_get_proc() == PDBG_PROC_P10 || pdbg_get_proc() == PDBG_PROC_PST) {
    ...
    pdbg_for_each_class_target(processor_class_name, chipTarget) {
      ...
      // If posState is set to VALID, check that our values match
      // If posState is set to WILDCARD, we don't care
      if ((index < 0) || ((i_target.posState == ECMD_TARGET_FIELD_VALID) &&
                          (index != i_target.pos)))
        continue;
...
uint32_t dllGetProcessingUnit(const ecmdChipTarget &i_target,
                              std::string &o_processingUnitName) {
  return ECMD_FUNCTION_NOT_SUPPORTED;
}

std::string dllLastError() { return "NOT_SUPPORTED"; }
...
uint32_t dllGetEcid(const ecmdChipTarget &i_target,
...
uint32_t dllPutGpRegisterUnderMask(const ecmdChipTarget &i_target,
                                   uint32_t i_gpRegister,
                                   const ecmdDataBuffer &i_data,
                                   const ecmdDataBuffer &i_mask) {
  return ECMD_FUNCTION_NOT_SUPPORTED;
}
...
/* ############################################################# */
uint32_t dllGetModuleVpdKeyword(const ecmdChipTarget &i_target,
                                const char *i_record_name,
                                const char *i_keyword, uint32_t i_bytes,
                                ecmdDataBuffer &o_data) {
  return ECMD_FUNCTION_NOT_SUPPORTED;
}

uint32_t dllPutModuleVpdKeyword(const ecmdChipTarget &i_target,
                                const char *i_record_name,
...
uint32_t dllGetModuleVpdKeywordFromImage(const ecmdChipTarget &i_target,
                                         const char *i_record_name,
                                         const char *i_keyword,
                                         uint32_t i_bytes,
                                         const ecmdDataBuffer &i_image_data,
                                         ecmdDataBuffer &o_keyword_data) {
  return ECMD_FUNCTION_NOT_SUPPORTED;
}

uint32_t dllPutModuleVpdKeywordToImage(const ecmdChipTarget &i_target,
                                       const char *i_record_name,
                                       const char *i_keyword,
                                       ecmdDataBuffer &io_image_data,
...
uint32_t dllGetFruVpdKeywordWithRid(const uint32_t i_rid,
                                    const char *i_record_name,
                                    const char *i_keyword, uint32_t i_bytes,
                                    ecmdDataBuffer &o_data) {
  return ECMD_FUNCTION_NOT_SUPPORTED;
...
uint32_t dllPutFruVpdKeywordWithRid(const uint32_t i_rid,
                                    const char *i_record_name,
                                    const char *i_keyword,
                                    const ecmdDataBuffer &i_data) {
  return ECMD_FUNCTION_NOT_SUPPORTED;
...
uint32_t dllGetFruVpdKeyword(const ecmdChipTarget &i_target,
                             const char *i_recordName, const char *i_keyword,
  ...
  if (!vpdInit) {
    return out.error(
        EDBG_CNFG_MISSING, FUNCNAME,
        "VPD functions not initialized!  Set EDBG_CNFG or run edbgdetcnfg.\n");
  }
...
uint32_t dllGetFruVpdKeywordFromImage(const ecmdChipTarget &i_target,
...
uint32_t dllPutFruVpdKeywordToImage(const ecmdChipTarget &i_target,
                                    const char *i_recordName,
                                    const char *i_keyword,
                                    ecmdDataBuffer &io_image_data,
                                    const ecmdDataBuffer &i_data) {
...
uint32_t dllGetRingSparseWithTraceMask(const ecmdChipTarget &i_target,
                                       const char *i_ringName,
                                       ecmdDataBuffer &o_data,
                                       const ecmdDataBuffer &i_mask,
                                       const ecmdDataBuffer &i_traceMask,
                                       uint32_t i_mode) {
  return ECMD_FUNCTION_NOT_SUPPORTED;
}
...
uint32_t dllGetSram(const ecmdChipTarget &i_target, uint32_t i_channel,
                    uint64_t i_address, uint32_t i_bytes,
                    ecmdDataBuffer &o_data) {
  return ECMD_FUNCTION_NOT_SUPPORTED;
}

uint32_t dllPutSram(const ecmdChipTarget &i_target, uint32_t i_channel,
                    uint64_t i_address, uint32_t i_bytes,
                    const ecmdDataBuffer &i_data) {
  return ECMD_FUNCTION_NOT_SUPPORTED;
}
...
uint32_t
dllQueryHostMemInfoRanges(const std::vector<ecmdChipTarget> &i_targets,
                          ecmdChipTarget &o_target,
 ...
 *         be returned if the execution fails.
 */
uint32_t iStepsHelper(uint16_t i_major_start, uint16_t i_major_end,
                      uint16_t i_minor_start, uint16_t i_minor_end) {
  uint32_t rc = ECMD_SUCCESS;
  ...
  /* kick off isteps */
  for (uint16_t major_istep = i_major_start; major_istep <= i_major_end;
       major_istep++) {
    ...
    if (major_istep < i_major_end) {
      l_minor_end = g_edbgIPLTable.getIStepMinorNumber(
          g_edbgIPLTable.getPosLastMinorNumber(major_istep));
    } else {
    ...
    if (major_istep > i_major_start) {
      l_minor_start = g_edbgIPLTable.getIStepMinorNumber(
          g_edbgIPLTable.getPosFirstMinorNumber(major_istep));
    } else {
      l_minor_start = i_minor_start;
    }

    for (uint16_t minor_istep = l_minor_start; minor_istep <= l_minor_end;
         minor_istep++) {

      auto l_istep_index_minor_end =
          g_edbgIPLTable.getPosLastMinorNumber(major_istep);
      auto l_istep_minor_end =
          g_edbgIPLTable.getIStepMinorNumber(l_istep_index_minor_end);
      ...
      if (l_destination == edbgIPLTable::EDBG_ISTEP_NOOP) {
        ...
        if (!rc) {
          if (major_istep == 6 && minor_istep == 4) {
            // if the istep reaches above 6.4  then, set the Host state to
            // running!
            rc = g_edbgIPLTable.setHostStateToRunning();
            if (rc != ECMD_SUCCESS)
              return out.error(rc, FUNCNAME,
                               "FAIL: failed to set host state\n");
          }
 ...
 *         be returned if the execution fails.
 */
uint32_t iStepsHelper(uint16_t i_major, uint16_t i_minor_start,
...
#ifdef EDBG_ISTEP_CTRL_FUNCTIONS
  uint32_t rc = ECMD_SUCCESS;
  uint16_t l_istep_index_begin,
      l_minor_start = edbgIPLTable::EDBG_INVALID_POSITION;
  ...
  do // Start of single exit point loop.
  {
    ...
    for (l_active_step = edbgIPLTable::EDBG_FIRST_ISTEP_NUM;
      ...
      if (i_steps.isBitSet(l_active_step)) // this is an active step
      {
        ...
        if (false == g_edbgIPLTable.isValid(
                         l_active_step)) { /* error - istep number l_active_step
                                              not found */

          /* this is only warning, as the value is in the range, but isn't being
           * used */
          out.warning(
              FUNCNAME,
        ...
        l_istep_index_begin =
            g_edbgIPLTable.getPosFirstMinorNumber(l_active_step);
         ...
         *    and ending with the ending entry #. */
        /* kick off isteps */
        rc = iStepsHelper(l_active_step, l_minor_start, l_minor_end);
...
uint32_t dllIStepsByName(std::string i_stepName) {
  ...
  transform(i_stepName.begin(), i_stepName.end(), i_stepName.begin(),
            (int (*)(int))tolower);
  ...
  if (false == g_edbgIPLTable.isValid(i_stepName)) {
    ...
    if (i_stepName == "list") {
      ...
      rc = dllQueryFileLocation(target, ECMD_FILE_HELPTEXT, l_fileLocs,
                                l_versionString);
      if (rc != ECMD_SUCCESS) {
        ...
        if (ins.fail()) {
          return out.error(ECMD_INVALID_ARGS, FUNCNAME,
                           "Error occured opening help file\n");
        } else {
...
uint32_t dllIStepsByNameMultiple(std::list<std::string> i_stepNames) {
  ...
  // order
  for (auto l_stepIter = i_stepNames.begin(); l_stepIter != i_stepNames.end();
       l_stepIter++) {
...
uint32_t dllIStepsByNameRange(std::string i_stepNameBegin,
  ...
  transform(i_stepNameEnd.begin(), i_stepNameEnd.end(), i_stepNameEnd.begin(),
            (int (*)(int))tolower);

  // Lookup i_stepNameBegin in IPLTable
  if (false == g_edbgIPLTable.isValid(
                   i_stepNameBegin)) { // error - i_stepNameBegin not found!
    rc = ECMD_ISTEPS_INVALID_STEP;
    return out.error(rc, FUNCNAME,
                     "Requested iStep-Begin not recognized:"
                     " %s. Returning rc=0x%x\n",
                     i_stepNameBegin.c_str(), rc);
  }

  // Get the index of the i_stepNameBegin in IPL Table
  g_edbgIPLTable.getIStepNumber(i_stepNameBegin, o_StartMajorNum,
                                o_StartMinorNum);
  l_istep_index_begin = g_edbgIPLTable.getPosFirstMinorNumber(o_StartMajorNum);

  // Lookup i_stepNameEnd in IPLTable
  if (false == g_edbgIPLTable.isValid(
                   i_stepNameEnd)) { // error - i_stepNameEnd not found!
    rc = ECMD_ISTEPS_INVALID_STEP;
    return out.error(rc, FUNCNAME,
                     "Requested iStep-End not recognized:"
                     " %s. Returning rc=0x%x\n",
  ...
  if (l_istep_index_begin >
      l_istep_index_end) { // error - i_stepName not found!
    rc = ECMD_ISTEPS_INVALID_STEP;
    return out.error(rc, FUNCNAME,
                     "Requested istep range '%s..%s' is invalid."
                     " %s. Returning rc=0x%x\n",
                     i_stepNameBegin.c_str(), i_stepNameEnd.c_str(), rc);
  }

  l_minor_start = g_edbgIPLTable.getIStepMinorNumber(
      g_edbgIPLTable.getPosition(i_stepNameBegin));

  l_minor_end = g_edbgIPLTable.getIStepMinorNumber(
      g_edbgIPLTable.getPosition(i_stepNameEnd));
...
uint32_t dllSyncIplMode(int i_unused) { return ECMD_FUNCTION_NOT_SUPPORTED; }
...
uint32_t dllQueryProcRegisterInfo(const ecmdChipTarget &i_target,
  ...
  // Translates to ecmd command: ecmdquery procregs Chip.chipunit
  // [procregistername]
  ...
  transform(l_name.begin(), l_name.end(), l_name.begin(),
            (int (*)(int))toupper);
...
uint32_t ecmdRegAccessHelper(ecmdChipTarget &i_target,
  ...
  pdbg_for_each_target("thread", core, thread) {
    ...
    for (l_ecmdIndexEntryIter = io_entries.begin();
         l_ecmdIndexEntryIter != io_entries.end(); l_ecmdIndexEntryIter++) {
      ...
      case ECMD_GET_GPR_ACCESS:
        ...
        if (rc != 0) {
          return out.error(EDBG_READ_ERROR, FUNCNAME,
                           "getgpr of 0x%016" PRIx64 " = 0x%016" PRIx64
                           " failed \n",
...
uint32_t ecmdSPRNameToIndex(const char *i_spr_name, const bool i_flag,
                            uint32_t &o_index) {
  uint32_t rc = ECMD_SUCCESS;
  bool l_mapInit = true;
...
uint32_t dllGetSprMultiple(const ecmdChipTarget &i_target,
  ...
  // ecmdDataBuffer and rc into the local entry;  then push that entry onto the
  // list
  for (l_nameEntryIter = io_entries.begin();
       l_nameEntryIter != io_entries.end(); ++l_nameEntryIter) {
    ecmdIndexEntry l_entry;

    // uppercase of SPR name and put it into l_entry.index
    transform(l_nameEntryIter->name.begin(), l_nameEntryIter->name.end(),
              l_nameEntryIter->name.begin(), (int (*)(int))toupper);

    uint32_t l_index;
    rc = ecmdSPRNameToIndex(l_nameEntryIter->name.c_str(), false, l_index);
    if (rc) {
      return out.error(rc, FUNCNAME, "GetSpr: Invalid SPR: %s\n",
  ...
  // Clear io_entries, as we're going to make a new ecmdNameEntry list that maps
  // to l_entryList
  io_entries.clear();

  // Now we map back the index to spr name and push back the results
  ecmdNameEntry l_nameEntry;
  for (l_indexEntryIter = l_entryList.begin();
       l_indexEntryIter != l_entryList.end(); ++l_indexEntryIter) {
...
uint32_t dllPutSprMultiple(const ecmdChipTarget &i_target,
  ...
  for (l_nameEntryIter = io_entries.begin();
    ...
    if (rc != 0) {
      return out.error(EDBG_GENERAL_ERROR, FUNCNAME, "Invalid SPR: %s\n",
                       l_nameEntryIter->name.c_str());
    }
    l_entry.index = l_index;
    l_entry.buffer = l_nameEntryIter->buffer;
